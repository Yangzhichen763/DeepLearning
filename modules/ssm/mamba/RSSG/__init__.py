...
# MambaIR: 基于Mamba的图像复原基准模型
# MambaIR: A simple baseline for image restoration with state-space model
# https://arxiv.org/pdf/2402.15648.pdf
# https://zhuanlan.zhihu.com/p/684248751
...
if __name__ == '__main__':
    ...
    width = 64
    L = 4096  # 64*64
    input_tensor = torch.randn(batch_size, L, channels)
    # 检查是否有可用的 GPU
    ...
    model = ResidualGroup(dim=channels, input_resolution=(height, width), depth=6, d_state=16, mlp_ratio=4.,
                          drop_path=0.)

    # 假设你的模型是 nn.Module 的一个实例
    model = model.to(device)

    # 使用输入张量调用模型
    output = model(input_tensor, (height, width))
    print(output.shape)