...
# TODO: 只做了誊抄，未进行测试并对理解代码
"""
Aggregated Pixel-Focused Attention(AA):
  - Pixel-Focused Attention：像素聚焦注意力机制，在每个Query附近具有细粒度的感知，同时保持全局信息的粗粒度意识
  - Aggregated Attention：是像素聚焦注意力的增强版，进一步将两种 non-QKV 注意力机制聚合到像素聚焦注意力中，附加计算量仅占总的 0.2%~0.3%

参考代码：https://github.com/DaiShiResearch/TransNeXt/blob/main/classification/attention_native.py
"""
...
class AggregatedAttention(nn.Module):
    ...
    def forward(self, x, H, W, relative_pos_index, relative_coords_table):
        ...
        pool_bias = (self.cpb_fc2(self.cpb_act(self.cpb_fc1(relative_coords_table)))
                     .transpose(0, 1)[:, relative_pos_index.view(-1)].view(-1, N, self.pool_len))
