    ...
    def p_sample(self, x_t, t, time_step, clip_denoised=True):
        ...
        def predict_xt_prev_mean_from_noise(pred_noise):
            return (
                ...
                - extract(self.posterior_mean_coeff_eps, t, x_t.shape) * pred_noise
            )
        ...
        def q_mean_std(x_0):
            ...
            # 后验均值
            posterior_mean = (
                extract(self.posterior_mean_coeff_x0, t, x_t.shape) * x_0
                + extract(self.posterior_mean_coeff_xt, t, x_t.shape) * x_t
            )
            # 后验方差
...
class DDIMSampler(GaussianDiffusionBase):
    ...
    """
    def __init__(self, t, model, betas, eta=0, accelerate=True):
        ...
        if accelerate:
            ...
            self.posterior_mean_coeff_xt = torch.sqrt(1. / self.alphas)
            self.posterior_mean_coeff_eps = (-torch.sqrt(1. - self.alphas_cumprod) / torch.sqrt(self.alphas)
    ...
    def forward(self, x_t, timesteps=None, clip_denoised=True):
        """
        Args:
            x_t (torch.Tensor): 原始高斯噪声图像
            timesteps (list): 时间步数列表，比如 t = 100 时可以取 timesteps = [10, 20, 30, 40, 50, 75, 100]
        ...
        if timesteps is None:
            timesteps = range(0, self.t)
        for i in tqdm(reversed(timesteps)):                  # 逐步加噪声
            t = x_t.new_ones((batch_size, ), dtype=torch.long) * i
            x_t = self.p_sample(x_t, t, i, clip_denoised)    # 反向采样
    ...
    def p_sample(self, x_t, t, time_step, clip_denoised=True):
        ...
        def q_mean_std(x_0, pred_noise):
            ...
            posterior_mean = (
                extract(self.posterior_mean_coeff_x0, t, x_t.shape) * x_0
                + extract(self.posterior_mean_coeff_eps, t, x_t.shape) * pred_noise   # direction pointing to x_t
            )
            # 后验标准差
            posterior_std = extract(self.sigma, t, x_t.shape)
            return posterior_mean, posterior_std
    ...
    def space_timesteps(self, section_strides, section_counts=None):
        """
        ...
        """
        size_per = self.t // len(section_strides)
        t_current = 0
        all_timesteps = [0]
...
if __name__ == '__main__':
    sampler = DDIMSampler(1000, None, linear(0, 1, 1000), eta=0, accelerate=True)
    _all_timesteps = sampler.space_timesteps([10, 15, 20, 25])
